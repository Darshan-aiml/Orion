...
interface GuidesSelectorProps {
  destinationId: string;
}

export function GuidesSelector({ destinationId }: GuidesSelectorProps) {
  const { selectedGuide, setGuide } = useTripStore((s) => ({
    selectedGuide: s.selectedGuide,
    setGuide: s.setGuide,
  }));
  ...
  return (
    ...
    >
      <Card
        ...
        onClick={() => onSelect(isSelected ? null : guide)}
      >
        ...
        <CardContent className="flex flex-col gap-3">
          ...
          <div className="flex flex-wrap gap-2">
            {guide.specializations?.slice(0, 3).map((s) => (
              <span
                key={s}
                className="inline-flex items-center gap-1 rounded-full bg-blue-50 dark:bg-blue-900/20 px-2 py-0.5 text-xs text-blue-700 dark:text-blue-400"
              >
          ...
          <Button
            ...
            onClick={(e) => {
              e.stopPropagation();
              onSelect(isSelected ? null : guide);
            }}
          >
