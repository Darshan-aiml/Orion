...
export function PackageBuilder({ destinationId }: PackageBuilderProps) {
  const { itinerary, addToItinerary, removeFromItinerary } = useTripStore((s) => ({
    itinerary: s.itinerary,
    addToItinerary: s.addToItinerary,
    removeFromItinerary: s.removeFromItinerary,
  }));
        ...
        <div className="flex flex-col gap-2 min-h-[200px]">
            ...
            {itinerary
              ...
              .map((item) => (
                ...
                >
                  ...
                  <button
                    onClick={() => removeFromItinerary(item.day, item.activityId)}
          ...
          {itinerary.filter((i) => i.day === selectedDay).length === 0 && (
            <p className="text-sm text-neutral-400 text-center py-8">
              No activities for Day {selectedDay}. Add some!
            </p>
          )}
        </div>
      </div>
...
function ActivityRow({
  ...
  activity: Activity;
  day: number;
  onAdd: (item: { day: number; activityId: string; activityName: string; price: number }) => void;
}) {
  return (
    <Card className="flex flex-row items-center justify-between p-4 gap-4">
      <div className="flex flex-col gap-1 flex-1">
        <div className="flex items-center gap-2">
          ...
          <p className="font-medium text-sm text-neutral-800 dark:text-neutral-100">{activity.name}</p>
        </div>
        <div className="flex items-center gap-3 text-xs text-neutral-400">
          {activity.duration_mins && (
            <span className="flex items-center gap-1">
              <Clock className="h-3 w-3" /> {activity.duration_mins}m
            </span>
          )}
          <span className="flex items-center gap-1">
            <DollarSign className="h-3 w-3" />{Number(activity.base_price).toFixed(2)}
          </span>
        </div>
      </div>
      <Button
        ...
        onClick={() =>
          onAdd({
            ...
            price: Number(activity.base_price),
          })
        }
      >
        <Plus className="h-3.5 w-3.5 mr-1" /> Add to Day {day}
      </Button>
    </Card>
  );
}
